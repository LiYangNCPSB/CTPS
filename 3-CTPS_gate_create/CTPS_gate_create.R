...
#input the cell type to purification
desired.cluster <- "Artery Cell"
...
channel.read <- read.csv("../2-CTPS_feature_selection/Selected_feature.csv", row.names = 1)
...
write.csv(gate_sol,"gate_result.csv")
...
for (i in 1:nrow(ngc)){
  ...
  pdf(file = paste("gate_figure/",rowgate,".pdf"))
  cal.gate.efficacy(trainingdata = data, bool.desired = bool.desired, gates = gate.ori, dims = dims, plot = T, verbose = T) #plot gate and print gate stats
  dev.off()
}
