...
plot.gate <- function(data = data, bool.desired = bool.desired, channels = channels, gate = gate, final.undesired = final.undesired, index = i) {
  ...
  plot(rbind(d2.desired, d2.undesired), cex.lab = 1.5, cex.axis = 1.3, xlim=c(0, 5), ylim = c(0, 5))
  ...
  if (length(gates) == 1) {
    ...
    if (verbose & plot) {
      ...
      if (plot){
        ...
        plot(c(1))
        text(1,1,paste("% of desired cells",sprintf("%0.2f",(sum(rowSums(valid.sc)== length(gates))/sum(bool.desired))*100)))
        text(1,1.2,paste("Purity",sprintf("%0.2f",(yield/(yield + impurity))*100)))
      }
    }
    else {
      return(c(yield.p, (yield/(impurity + yield))))
